...
func Test_CreateUnidentifiedToken(t *testing.T) {
	token := tokens.CreateUnidentifiedToken("the text", 1, 2, 3)
	assert.Equal(t, 1, token.LineNumber)
	assert.Equal(t, 2, token.TabNumber)
	assert.Equal(t, 3, token.BracketNumber)
...
func Test_Equal_True(t *testing.T) {
	token1 := tokens.CreateUnidentifiedToken("the text", 1, 2, 3)
	token1.SetValues("KEYWORD", "IF")

	token2 := tokens.CreateUnidentifiedToken("the text", 1, 2, 3)
